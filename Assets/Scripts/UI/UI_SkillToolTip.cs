...
public class UI_SkillToolTip : UI_ToolTip
{
    ...
    protected override void Awake()
    {
        ...
        skillTree = ui.GetComponentInChildren<UI_SkillTree>(true); // (true) makes sure to get component even if it's inactive
    }
    ...
    private string GetRequirements(int skillCost, UI_TreeNode[] neededNodes, UI_TreeNode[] confictNodes)
    {
        ...
        if(confictNodes.Length <= 0)
            return sb.ToString();
        
        sb.AppendLine();
        sb.AppendLine(GetColoredText(importantInfoHex, "Locks out:"));
        foreach (var node in confictNodes)
        {
            string nodeText = $"- {node.skillData.displayName}";
